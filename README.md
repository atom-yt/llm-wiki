...
- **三层语义检索**：
  - QMD CLI 语义搜索（如果已安装）
  - SimpleEmbedder TF-IDF 嵌入（无需外部依赖）
  - BM25 关键词搜索（自动回退）
- **交互式 Ingest**：分步审查并批准 LLM 生成的更改
...
语义搜索优先级：
1. QMD CLI（需额外安装）
2. SimpleEmbedder TF-IDF（内置，无需外部依赖）
3. BM25 关键词（自动回退）
...
| `llm-wiki schema --analyze` | 分析 Wiki 结构并建议改进 |
...
| POST | `/api/ingest/start` | 开始交互式 ingest |
| POST | `/api/ingest/propose` | 生成页面提案 |
| POST | `/api/ingest/apply` | 应用批准的更改 |
| POST | `/api/query` | 查询知识库 |
| POST | `/api/qmd/index` | 建立语义索引 |
| GET | `/api/qmd/status` | 获取搜索状态 |
| POST | `/api/lint` | 健康检查 |
...
- **Three-tier semantic search**:
  - QMD CLI semantic search (if installed)
  - SimpleEmbedder TF-IDF embeddings (no external dependencies)
  - BM25 keyword search (automatic fallback)
- **Interactive ingest**: Step-by-step review and approve LLM-generated changes
...
### 3.5. Build Semantic Index (Recommended)

```bash
# Build semantic index for better search quality
llm-wiki index
```

Semantic search priority:
1. QMD CLI (requires separate installation)
2. SimpleEmbedder TF-IDF (built-in, no external dependencies)
3. BM25 keyword search (automatic fallback)
...
| `llm-wiki index [--force]` | Build semantic search index |
| `llm-wiki query <question>` | Query knowledge base |
| `llm-wiki lint [--fix]` | Health check |
| `llm-wiki schema --analyze` | Analyze Wiki structure and suggest improvements |
...
- **Interactive Ingest**: Step-by-step review and approve LLM-generated changes
- **Query**: Natural language queries to knowledge base with search mode status
...
| POST | `/api/ingest/start` | Start interactive ingest |
| POST | `/api/ingest/propose` | Generate page proposals |
| POST | `/api/ingest/apply` | Apply approved changes |
| POST | `/api/query` | Query knowledge base |
| POST | `/api/qmd/index` | Build semantic index |
| GET | `/api/qmd/status` | Get search status |
